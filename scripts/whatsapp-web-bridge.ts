...
function validarEntorno(
  ...
  obtenerSupabaseServiceRoleKey: () => string | undefined
  ...
  if (!existsSync(envPath)) {
    ...
    console.warn(
      ...
      `[whatsapp-web] No se encontró ${envPath}; usando variables del sistema.`
...
async function main() {
  ...
  const [
    ...
    import("../lib/messaging/index"),

    import("../lib/whatsapp/whatsapp-service"),

    import("../lib/supabase-server-key"),
  ...
  const { registrarMessagingProvider, WhatsAppWebProvider } = messagingModule;

  const { obtenerWhatsAppService } = whatsappServiceModule;

  const { obtenerSupabaseServiceRoleKey } = supabaseKeyModule;



  validarEntorno(obtenerSupabaseServiceRoleKey);
  ...
  console.log(
    ...
    "[whatsapp-web] Iniciando whatsapp-web.js (primera vez puede tardar 1–2 min)…\n"
