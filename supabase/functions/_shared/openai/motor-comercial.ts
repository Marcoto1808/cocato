...
export type LineaInterpretadaComercial = {
  producto_id: string;
  cantidad: number;
  unidad: "kg" | "pieza";
...
export type InterpretacionComercial =
  | {
      tipo: "pedido";
      lineas: LineaInterpretadaComercial[];
      observaciones?: string[];
      aclaracion?: string;
      disambiguacion?: DisambiguacionPendiente;
    }
  | { tipo: "no_interpretado"; motivo: string }
  | { tipo: "referencia_historica"; motivo: string };
...
const NUMEROS_EN_PALABRAS = new Set([
  "uno", "una", "un", "dos", "tres", "cuatro", "cinco", "seis", "siete", "ocho",
  "nueve", "diez", "once", "doce", "trece", "catorce", "quince", "veinte",
  "treinta", "cuarenta", "cincuenta", "medio", "media",
]);
...
function aclaracionCantidadSinProducto(unidad?: "kg" | "pieza" | null): string {
  if (unidad === "kg") return "¿Qué producto necesita? Ejemplo: 5 kilos de costilla";
...
function aclaracionProductoSinCantidad(producto: ProductoCatalogo): string {
  if (producto.unidad === "kg") {
    return [`¿Cuántos kilos de ${producto.nombre} necesita?`, "", "Ejemplos: 5, 8.5"].join("\n");
  }
  return [`¿Cuántas piezas de ${producto.nombre} necesita?`, "", "Ejemplos: 1, 2, 1.5"].join("\n");
}
...
function motivoDesdeAnalisis(analisis: Exclude<SegmentoAnalizado, { tipo: "ok" }>): string {
  if (analisis.tipo === "ambiguo") return aclaracionDesdeDisambiguacion(analisis);
  if (analisis.tipo === "falta_producto") return aclaracionCantidadSinProducto(analisis.unidad);
  if (analisis.tipo === "falta_cantidad") return aclaracionProductoSinCantidad(analisis.producto);
...
function detectarSoloCantidad(
  ...
  if (/^\d+(?:[.,]\d+)?$/.test(normalizado)) return { esSoloCantidad: true, unidad: null };
  if (NUMEROS_EN_PALABRAS.has(normalizado)) return { esSoloCantidad: true, unidad: null };
  if (/^(medio|media)$/.test(normalizado)) return { esSoloCantidad: true, unidad: null };
  if (/^(dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\s+y\s+(medio|media)$/.test(normalizado)) {
...
function pareceImporteSinUnidad(importe: number, texto: string): boolean {
  if (!Number.isFinite(importe) || importe < IMPORTE_MINIMO_SIN_PESOS) return false;
  const normalizado = normalizarTextoPedido(texto);
  if (/\b(kg|kilos?|kilo|piezas?|pz|pza)\b/.test(normalizado)) return false;
  return true;
}
...
function parsearSegmentoImporte(texto: string): {
  ...
  if (conSimbolo) {
    ...
    return { cantidadTexto: `$${conSimbolo[1]}`, productoTexto };
  }
  ...
  if (conPesos) {
    ...
    if (!Number.isFinite(importe) || importe <= 0 || !productoTexto) return null;
    return { cantidadTexto: `${conPesos[1]} pesos`, productoTexto };
  }
  ...
  if (sinPesos) {
    ...
    return { cantidadTexto: `${sinPesos[1]} pesos`, productoTexto };
  }
...
function analizarSegmento(
  ...
  if (soloCantidad.esSoloCantidad) {
    return { tipo: "falta_producto", unidad: soloCantidad.unidad, segmento };
  }
  ...
  if (importe) {
    const resolucion = resolverProductoEnCatalogo(importe.productoTexto, productos, contexto);
  ...
  if (parsed) {
    const resolucion = resolverProductoEnCatalogo(parsed.productoTexto, productos, contexto);
    const interpretado = resolucionComoSegmento({
      segmento,
  ...
  if (productoSolo) return { tipo: "falta_cantidad", producto: productoSolo, segmento };
...
function lineaLibreDesdeSegmento(segmento: string): LineaInterpretadaComercial {
...
}): InterpretacionComercial {
  const texto = limpiarPrefijoPedido(input.texto.trim());
  if (!texto) return { tipo: "no_interpretado", motivo: "Mensaje vacío." };
  if (requiereIa(texto)) {
    return { tipo: "referencia_historica", motivo: "Requiere interpretación avanzada (fase IA)." };
  }

  const lineas: LineaInterpretadaComercial[] = [];
