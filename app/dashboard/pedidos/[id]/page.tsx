...
import type { LineaPedido } from "@/lib/pedido-productos";
import {
  PedidoEstadoBadge,
  PedidoEstadoBotones,
...
export default async function PedidoDetallePage({ params }: Props) {
  ...
  const [{ data: pedido, error }, { data: todosPedidos }, { data: lineas }] =
    await Promise.all([
      supabase
        .from("pedidos")
        .select(
          "id, cliente_id, estado, fecha, mensaje_original, observaciones, total, clientes(nombre_negocio, propietario, telefono, whatsapp, direccion)"
        )
        .eq("id", id)
        .single(),
    ...
    <PedidoEstadoProvider pedidoId={id} estadoInicial={detalle.estado}>
      <main className="min-h-screen bg-zinc-100 p-8">
        ...
        <div className="mx-auto max-w-2xl space-y-4">
          ...
          <PedidoProductos
            pedidoId={id}
            lineasIniciales={(lineas ?? []) as LineaPedido[]}
          />
